...
// Nivel a comparar contra nivel_min: para Ueno usamos el nivel ueno+ del usuario;
// para el resto, la escala genérica (Clásica→Infinite).
const nivelParaBeneficio = (t, beneficio) =>
  (beneficio.banco_id === UENO_ID && t.ueno_nivel) ? t.ueno_nivel : nivelDeTarjeta(t);
...
export function tarjetaQueAplica(beneficio, misTarjetas) {
  ...
  return misTarjetas.find(t => {
    ...
    // nivel mínimo (Ueno: compara contra el nivel ueno+ del usuario)
    if (nivelMin && nivelParaBeneficio(t, beneficio) < nivelMin) return false;
    return true;
  }) || null;
}
